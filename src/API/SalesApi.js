...
import { setCustomerDropDown, setProductDropDown, setSales, setTotal } from "../slices/saleSlice";
...
// sales List Api
export async function ListSalesApi(pageNumber, perPage, searchKeyword) {
    let url = BASE_URL  + `/sales-list/${pageNumber}/${perPage}/${searchKeyword}`
    ...
    try{
        ...
        if(result.status===200 && result.data.status==="success"){
            if(result.data.data[0].data.length>0){
                store.dispatch(setSales(result.data.data[0].data))
...
// Create Sales Api
export async function SalesCreateApi (parentBody , childBody){
...
// Delete sales Api
export async function DeleteSalesApi(id) {

    try{
        let url = BASE_URL + `/sales-delete/${id}`;
