...
import { DeleteBrandApi, ListBrandApi } from "../API/BrandApi";
...
const BrandList = () => {
    ...
    useEffect(() => {
        ListBrandApi(1, perPage, searchKeyword)
    }, [])


    const allBrand = useSelector((state) => state.brand.brands)
    
    const total = useSelector((state) => state.brand.list[0]?.count)
    ...
    const handlePageChange = (pageNumber) => {
       ...
       ListBrandApi(pageNumber, perPage, searchKeyword)
    }

    const handlePerPage=(e)=>{
        setPerPage(parseInt(e.target.value))
        setCurrentPage(1)
        ListBrandApi(1, parseInt(e.target.value), searchKeyword)
    }
    ...
    const handleSearchValue=(e)=>{
        ...
        if(e.target.value.length===0){
            ...
            ListBrandApi(currentPage, perPage, "null")
        }
    }
    ...
    return (
        ...
        <div className="bg-purple-500 p-4 flex justify-between items-center mb-5 mt-3">
          <h1 className="text-white font-bold text-3xl">Brand List</h1>
          ...
          <div className="flex gap-5 items-center">
            ...
            <div>
              <Link to="/brand-create-update" className="btn btn-warning">Create Brand</Link>
            </div>
          </div>
        </div>
        {/* =============== Header End =============== */}
      ...
      <table className="min-w-full bg-white mb-5">
                  <thead className="bg-purple-600 text-white">
                      <tr>
                          ...
                          <th className="w-2/12 py-2 px-4 text-left">Brand Name</th>
  ...
  
                      {
                          allBrand?.map((brand, i) => {
                              ...
                              <tr key={i} className="bg-gray-100 border-b">
                                  ...
                                  <td className="py-2 px-4">{moment(brand.createdDate).format("DD-MM-YYYY")}</td>
                                  <td className="py-2 px-4">
                                  <button className=" text-white px-2 py-1 rounded flex items-center gap-2 justify-end">
                                  <Link to={`/brand-create-update/${brand._id}`} ><CiEdit className="text-[35px] text-purple-600" /></Link>
                                  <MdDeleteForever onClick={() => deleteExpenseType(brand._id)} className="text-4xl text-red-500" />
                                  </button>
                                  </td>
                              </tr>
                              )
