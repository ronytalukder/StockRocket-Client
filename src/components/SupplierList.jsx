...
import { DeleteSupplierApi, SupplierListApi } from "../API/SuppliserApi";
...
const SupplierList = () => {
    ...
    useEffect(() => {
        SupplierListApi(1, perPage, searchKeyword)
    }, [])


    const allSuppliers = useSelector((state) => state.supplier.suppliers)
    const total = useSelector((state) => state.supplier.list[0]?.count)
    ...
    const handlePageChange = (pageNumber) => {
       ...
       SupplierListApi(pageNumber, perPage, searchKeyword)
    }

    const handlePerPage=(e)=>{
        setPerPage(parseInt(e.target.value))
        setCurrentPage(1)
        SupplierListApi(1, parseInt(e.target.value), searchKeyword)
    }
    ...
    const handleSearchValue=(e)=>{
        ...
        if(e.target.value.length===0){
            ...
            SupplierListApi(currentPage, perPage, "null")
        }
    }
    ...
    return (
        <div>
          ...
          <div className="bg-purple-500 p-4 flex justify-between items-center mb-5 mt-3">
            <h1 className="text-white font-bold text-3xl">Supplier List</h1>
            ...
            <div className="flex gap-5 items-center">
              ...
              <div>
                <Link to="/supplier-create-update" className="btn btn-warning">Create Supplier</Link>
              </div>
            </div>
          </div>
          {/* =============== Header End =============== */}
        ...
        <table className="min-w-full bg-white mb-5">
                    <thead className="bg-purple-600 text-white">
                        <tr>
                            ...
                            <th className="w-2/12 py-2 px-4 text-left">Supplier Name</th>
    ...
    
                        {
                            allSuppliers?.map((supplier, i) => {
                                ...
                                <tr key={i} className="bg-gray-100 border-b">
                                    ...
                                    <td className="py-2 px-4">{moment(supplier.createdDate).format("DD-MM-YYYY")}</td>
                                    <td className="py-2 px-4">
                                    <button className=" text-white px-2 py-1 rounded flex items-center gap-2 justify-end">
                                    <Link to={`/supplier-create-update/${supplier._id}`} ><CiEdit className="text-[35px] text-purple-600" /></Link>
                                    <MdDeleteForever onClick={() => deleteSupplier(supplier._id)} className="text-4xl text-red-500" />
                                    </button>
                                    </td>
                                </tr>
                                )
